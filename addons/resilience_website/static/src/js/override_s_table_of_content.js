odoo.define('resilience_website.s_table_of_content_options', function (require) {
  'use strict';
    ...
    _generateNav: function (ev) {
      ...
      _.each(headingsEls, el => {
        ...
        if (el.tagName.toLowerCase() === 'h1') {
          ...
          .addClass('heading-1 table_of_content_link list-group-item list-group-item-action py-2 border-0 rounded-0')
          .text($el.text())
          .appendTo(navEl);
        } else if (el.tagName.toLowerCase() === 'h2') {
          $('<a>').attr({'href': "#" + id, 'data-visibility-id': visibilityId})
          .addClass('heading-2 table_of_content_link list-group-item list-group-item-action py-2 ps-3 border-0 rounded-0')
          .text($el.text())
          .appendTo(navEl);
        } else if (el.tagName.toLowerCase() === 'h3') {
          $('<a>').attr({'href': "#" + id, 'data-visibility-id': visibilityId})
          .addClass('heading-3 table_of_content_link list-group-item list-group-item-action py-2 ps-4 border-0 rounded-0')
          .text($el.text())
          .appendTo(navEl);
        }
        $el.attr('id', id);
